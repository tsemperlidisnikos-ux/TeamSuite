...
Push-Location $twa
try {
  .\gradlew.bat assembleDebug --no-daemon -PparentApp
  if ($LASTEXITCODE -ne 0) { throw "Gradle assembleDebug -PparentApp failed" }
} finally {
  Pop-Location
}

$apk = Join-Path $twa "app\build\outputs\apk\debug\app-debug.apk"
if (-not (Test-Path $apk)) { throw "APK not found after parent build" }

$dest = Join-Path $backupDir "TeamSuite-Goneis.apk"
