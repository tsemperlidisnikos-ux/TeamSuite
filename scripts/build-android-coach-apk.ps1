...
Push-Location $twa
try {
  .\gradlew.bat assembleDebug --no-daemon -PcoachApp
  if ($LASTEXITCODE -ne 0) { throw "Gradle assembleDebug -PcoachApp failed" }
} finally {
  Pop-Location
}

$apk = Join-Path $twa "app\build\outputs\apk\debug\app-debug.apk"
if (-not (Test-Path $apk)) { throw "APK not found after coach build" }

$dest = Join-Path $backupDir "TeamSuite-Pronohtes.apk"
